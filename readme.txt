...
= 1.10.15 =
*   ✨ New Feature: Added "Align on Rail" button to evenly distribute cards along selected rails
*   📏 Smart Distribution: Automatically calculates equal spacing for all cards connected to a rail
*   🎯 Precision Alignment: Cards are positioned at exact intervals with centered positioning
*   🔗 Connection Updates: Automatically adjusts connection anchors to match new positions
*   🎨 UX: Simple one-click alignment for professional-looking layouts
*   💡 Visual Feedback: Toast notifications guide the user through the process
*   ⚡ Auto-Save: Automatic save after alignment with full history tracking for undo
*   🛡️ Validation: Checks for rail selection and minimum card count before processing

= 1.10.14 =
*   🐛 Critical Fix: Rail connections now attach exactly where you click
*   🎯 Precision: Fixed anchor positioning to maintain precise click coordinates on rails
*   🔧 Improved: Vertical rails now keep Y-position precise, X centered
*   🔧 Improved: Horizontal rails now keep X-position precise, Y centered
*   ✨ Enhancement: Connections no longer "jump" to unexpected positions on rails
*   🎨 UX: Visual consistency between connection preview and final connection placement

= 1.10.13 =
*   ✨ New Feature: Added "Duplicate Card" button to each card's settings panel
*   🎯 Enhancement: One-click duplication of individual cards with all properties
*   🔗 Smart Duplication: Automatically copies all connections from the original card
*   📋 Properties Copied: Image, caption, title, link, target, style, and connection style
*   🎨 UX: Duplicated cards offset by 50px for easy visual distinction
*   ⚡ Auto-Save: Automatic save after duplication with history tracking
*   💡 Visual Feedback: Toast notification confirms successful duplication
*   🎨 Professional Styling: Clean button design with icon and hover effects
...
*   🎨 Enhancement: Combines flowchart routing with arrows and dashed lines for better visual distinction
*   🔧 Code Quality: Implemented in both admin and frontend with full arrow reversal support
...
*   🔧 Critical Fix: Removed orphaned object literals left behind from automated cleanup
*   ✅ Stability: All JavaScript files now error-free and fully functional
*   🎯 Quality: Improved code cleanup process to prevent similar issues
